...
public class ClusterBenchmarker {
    ...
    public static void main(String[] args) {
        ...
        File activeDir = new File("activemqtests/");
        File[] listOfActive = activeDir.listFiles();
        List<String> activeMQTests = new ArrayList<>();
        for(File f : listOfActive){
            activeMQTests.add(f.getAbsolutePath());
        }

        if(activeMQTests.isEmpty()){
            System.exit(1);
        }
        for(String testFile : activeMQTests){
